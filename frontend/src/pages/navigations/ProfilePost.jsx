        ...
        <div className="profile-container">
          <div className="profile-header flex">
            ...
            <div className="details flex">
              <h2 className="name">{userPosts.user.fullName}</h2> {/* Displaying full name */}
              <p className="email">{userPosts.user.email}</p> {/* Displaying email */}
            </div>
          </div>

          <div className="bio-container flex">
            <h3 className="bio-title">Bio:</h3>
            <p className="bio">{userPosts.user.bio || 'This user has not written a bio yet.'}</p> {/* Display bio or default text */}
          </div>

          <div className="location-container flex">
            <p className="location">
              Location:
              Latitude: {userPosts.user.location ? userPosts.user.location.coordinates[1] : 'N/A'},
              Longitude: {userPosts.user.location ? userPosts.user.location.coordinates[0] : 'N/A'}
            </p> {/* Displaying location if available */}
          </div>
        </div>
